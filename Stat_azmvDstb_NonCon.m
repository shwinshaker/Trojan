...
%     thetav=atan(yv./xv);
%     
%     xv=(V_rlt(:,1).^2+V_rlt(:,2).^2).^(1/2);
%     yv=V_rlt(:,3);
%     thetavNC=atan(yv./xv);

    thetav=acos(sum(Vp_con.*Vt_con,2)./(sum(Vp_con.*Vp_con,2).^(1/2).*sum(Vt_con.*Vt_con,2).^(1/2)));
    thetavNC=acos(sum(Vp.*Vt,2)./(sum(Vp.*Vp,2).^(1/2).*sum(Vt.*Vt,2).^(1/2)));
    ...
    countv=histcounts(thetav,thetax)';
    countvNC=histcounts(thetavNC,thetax)';

    plotx=(thetax(2:end)-dtheta/2)/pi*180;
    ploty=countv/length(thetav);
    plotyNC=countvNC/length(thetavNC);
    
    disp(titlename);

%     bar(plotx,ploty,'facecolor',[0.9 0.9 0.9],'edgecolor','none');
    h2=plot(plotx,plotyNC,'k.-','linewidth',1.3,'markersize',10);
    h1=plot(plotx,ploty,'r.-','linewidth',1.3,'markersize',10);
